...
配置只存在本机，不会上传。密钥类字段保存后再次打开会显示掩码，留空表示不修改。换电脑可用 [导入与导出](#导入与导出配置)。
...
列表项可打开、重命名、删除；本地草稿可点纸飞机图标发布到 `source/_posts`。顶部搜索框按标题、文件名、路径过滤。拖动列表右边缘可调整宽度（会记住）；鼠标悬停会显示完整标题。
...
Hexo 的草稿在 `source/_drafts`，发布就是把文件（以及同名资源目录）移到 `source/_posts`。在列表里点纸飞机，或菜单 **文件 → 发布草稿**。若已发布目录有同名文件，会自动加序号。开启「保存后自动 SFTP 上传」且已连接 SSH 时，发布后会推到服务器。
...
1. 已填好 SSH 时，**打开应用会自动连接**。也可点远程栏插头，或菜单 **远程 → 连接 SSH**。连上后状态栏和远程栏会显示 `用户@主机`。
...
1. **导出**：菜单 **文件 → 导出配置…**，或 **设置** 底部的「导出配置…」。会生成 JSON（默认放到「文档」）。导出的是**已经保存**的内容，含 R2 / SSH / LLM 密钥和文章模板。
2. **导入**：选该 JSON，确认覆盖后写入本机。也可直接导入 `data/config.json`（不含模板）。
3. 换电脑后，若博客目录或 SSH 私钥路径不同，导入后再在设置里改一次。

文件含明文密钥，请只放在本机或加密盘，不要提交到 Git。
...
**「已发布」是空的，或提示未连接**  
先在设置里填好 SSH 并保存。下次打开会自动连接；也可点远程栏插头。已发布列表读的是**服务器**上的文章，不是本地 `_posts`。
